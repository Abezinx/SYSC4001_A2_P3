...
#include<interrupts_101266157_101313150.hpp>
...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            // Simulate the scheduler call (same as sample output in assignment)
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////
            // Simulate loading and execution steps for the EXEC call

            // Find program size from external_files table
            unsigned int program_size = get_size(program_name, external_files);

            // Log size info (matches assignment example)
            execution += std::to_string(current_time) + ", 50, Program is " + std::to_string(program_size) + "Mb large\n";
            current_time += 50;

            // Simulate loading the program into memory (15ms per MB)
            ...
            // Mark partition as occupied (memory allocation)
            execution += std::to_string(current_time) + ", 3, marking partition as occupied\n";
            current_time += 3;

            // Update the PCB with the new program info
